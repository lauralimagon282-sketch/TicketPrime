# TicketPrime - Motor de Vendas de Ingressos

Alunos:
Laura de Lima - 06010735
Carlos Eduardo Mendes - 06011992
Caio de Paiva Barragat - 060
Arthur Martins - 060
Emanuel de Oliveira - 06010524

Este é o backend do sistema TicketPrime, desenvolvido como uma Minimal API em .NET utilizando Dapper para persistência de dados.

## Como Executar o Projeto

*Necessário MICROSOFT SQL SERVER MANAGEMENT STUDIO* Para compilar e rodar a API, certifique-se de ter o SDK do .NET instalado, navegue até a pasta do código-fonte e execute:

```bash
cd src
dotnet run
```
/*
    LOGIN DO ADMINISTRADOR:
   E-mail: admin@prime.com
   Senha: 123456
*/
